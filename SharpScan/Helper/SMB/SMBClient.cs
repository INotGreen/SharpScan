...
namespace SharpScan.Helper.SMB
{
    public class SmbClient : IDisposable
    {
        ...
        {
             var fs = File.OpenWrite(localFile);
        ...
        private void CheckStatus(NTStatus status, string method, params string[] args)
        {
            if (status != NTStatus.STATUS_SUCCESS)
            {
                string argsString = args.Length > 0 ? string.Join("", args.Select(f => $"[{f}]").ToArray()) : "";
                throw new Exception($"Error Occurs:{method}-{status}" + argsString);
            }
        }
        ...
        private string FormatPath(string workingDirectory, string path)
        {
            ...
            // 将 IEnumerable<string> 转换为 string[]
            return string.Join(seperator, paths.Where(f => !string.IsNullOrEmpty(f)).ToArray());
        }
        ...
        private SmbClientFile[] GetList(string path, string pattern, bool? directory)
        {
            ...
            if (store is SMB1FileStore fileStore)
            {
                ...
                Action<FindFileDirectoryInfo> action = directory.HasValue ?
                    (directory.Value ?
                        (Action<FindFileDirectoryInfo>)(fileDirectoryInfo =>
                        {
                            if ((fileDirectoryInfo.ExtFileAttributes & ExtendedFileAttributes.Directory) == ExtendedFileAttributes.Directory)
                            {
                    ...
                    (fileDirectoryInfo =>
                    {
                        var isDirectory = (fileDirectoryInfo.ExtFileAttributes & ExtendedFileAttributes.Directory) == ExtendedFileAttributes.Directory;
                        var modified = fileDirectoryInfo.LastWriteTime ?? fileDirectoryInfo.CreationTime;
            ...
            else
            {
                ...
                Action<FileDirectoryInformation> action = directory.HasValue ?
                    (directory.Value ?
                        (Action<FileDirectoryInformation>)(file =>
                        {
                            if ((file.FileAttributes & SMBLibrary.FileAttributes.Directory) == SMBLibrary.FileAttributes.Directory)
                            {
                    ...
                    (file =>
                    {
                        files.Add(new SmbClientFile(file.FileName, (file.FileAttributes & SMBLibrary.FileAttributes.Directory) == SMBLibrary.FileAttributes.Directory, file.ChangeTime, file.CreationTime));
                    });
